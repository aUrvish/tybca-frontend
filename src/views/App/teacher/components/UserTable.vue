...
const { getTeachersAction, getTeachersSeacherAction, removeAction, disableAction, getTeachersFilterAction } = useAuthStore();
...
onMounted(
    () => {
        ...
        getTeachersAction(pagination.value)
            .then(
                (res) => {
...
watch(
    ...
    debounce((newVal) => {
        ...
        if (newVal) {
            changeStatusLoading(true)
            getTeachersFilterAction({ course_id: newVal })
                .then(
                    (res) => {
                        if (res.data.data) {
                            teacherList.value = res.data.data;
...
const removeUserFunc = (id) => {
    ...
    removeAction(id)
        .then(
            (res) => {
                teacherList.value.data = teacherList.value.data.filter((curr) => curr.id != id);
                changeStatusLoading(false)
            }
        )
        .catch(
    ...
    <section class="bg-gray-50 pt-4">
        <div>
            ...
            <div class="bg-white relative border sm:rounded-lg overflow-hidden">
                <div class="flex items-center gap-2 justify-between p-4 border-b">
                    ...
                    <div class="flex md:gap-4 gap-2 items-center">
                        <div class="w-full sm:max-w-[200px] max-w-[150px]">
                            <select v-model="filterCourse"
                                ...
                                <option :value="co.id" v-for="(co, ind) in course" :key="ind">{{ co.name }}</option>
                            </select>
                        </div>
                        <div class="w-full sm:max-w-[200px] max-w-[150px]">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody>
                            <tr class="[&>*]:whitespace-nowrap border-b" v-for="(teacher, index) in teacherList.data"
                                :key="index">
                                <td scope="row" class="px-4 py-3">
                                    <div class="flex items-center gap-4">
                                        <div class="w-8 aspect-square overflow-hidden border rounded-full">
                                            <img :src="getStorage + teacher.avatar" class="object-cover w-full h-full"
                                ...
                                Date(teacher.created_at).toLocaleDateString().replaceAll("/", "-") }}</td>
                                <td class="px-4 py-3 text-center">
                                    <span class="text-[12px] border text-success rounded-md border-success py-1 px-2"
                                        v-if="teacher.status">Active</span>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex justify-between items-center gap-2">
                                        ...
                                        <label class="inline-flex items-center cursor-pointer"
                                            ...
                                            <input type="checkbox" class="sr-only peer" :checked="teacher.disable"
                                                @change="changeStatus($event.target.checked, teacher.id)">
                                            <div
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" class="fill-blue cursor-pointer w-4"
                                            viewBox="0 0 512 512" @click="router.push(`/profile/${teacher.id}`)">
                                            <path
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" class="fill-success cursor-pointer w-4"
                                            viewBox="0 0 512 512" v-if="auth.user.role_id == 0"
                                            @click="router.push(`/profile/edit/${teacher.id}`)">
                                            <path
                    ...
                    <p v-if="teacherList.data?.length == 0" class="px-4 py-2 border-b" >No Data Found</p>
                </div>
                <nav class="flex justify-between items-center space-y-0 p-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500">
                        Result
                        <span class="font-semibold text-gray-900">{{ teacherList.total }}</span>
                    </span>
                    <ul class="inline-flex items-stretch -space-x-px">
                        <li>
                            <button
                                :class="teacherList.prev_page_url ? 'cursor-pointer hover:bg-gray-100 hover:text-gray-700' : 'bg-slate-100 cursor-auto'"
                                @click="teacherList.prev_page_url ? pagination-- : ''"
                        ...
                        <li>
                            <button
                                :class="teacherList.next_page_url ? 'cursor-pointer hover:bg-gray-100 hover:text-gray-700' : 'bg-slate-100 cursor-auto'"
                                @click="teacherList.next_page_url ? pagination++ : ''"
