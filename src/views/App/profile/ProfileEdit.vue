...
const { authProfilesEditAction, getAuthProfileAction } = useAuthStore();
...
const addUser = () => {
    invalideCource.value = selectCourse.value.length <= 0
    v$.value.$validate();
    if (v$.value.$invalid || (invalideCource.value && auth.value.user.role_id != 0)) {
        return null
    }
    ...
    authProfilesEditAction(formData)
        .then(
            (res) => {
        ...
        <div class="flex gap-4 items-start flex-col 3lg:flex-row max-w-screen-xl mx-auto">
            <div class="grow w-full">
                <div class="border rounded-md bg-white p-4 mb-4">
                    <h1 class="text-[24px] font-semibold">Profile</h1>
                </div>
            ...
            <div class="3lg:max-w-[396px]" :class="auth.user.role_id == 0 ? 'invisible' : ''">
